...
<body>
		...
		<div class="form-group">
			...
			<div class="col-sm-6">
				<select class="form-control" name="rule" id="rule">
					...
					<option value="2" ${data.userData.rule == '1'?' selected':''}>普通用户</option>
				</select>
			</div>
		</div>
		<div class="form-group">
        ...
        <div class="form-group">
			...
			<div class="col-sm-2">
				<select class="form-control" id="province" name="province">
					...
					<option
						value="<c:out value="${data.userData.city.father.provinceid}" />"><c:out
							value="${data.userData.city.father.province}" />
					</option>
					<%-- 					</c:forEach> --%>
				</select>
				<button type="button" id="changeProvince">更改省份</button>
			</div>
			...
			<div class="col-sm-6">
				<select class="form-control" id="certType" name="certType">
					<c:forEach items="${certTypeData.data}" var="s" varStatus="s1">
						<option value="<c:out value="${s.ID}"/>" ${data.userData.rule == '1'?' selected':''}><c:out
								value="${s.content}" />
						</option>
					</c:forEach>
				</select>
 ...
 -->
					<input type="button" class="btn btn-danger btn-large btn-block" id="uSaveBtn" value="添加"></button>
			</div>
		</div>
	</form>
